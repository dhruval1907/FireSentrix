...
import { guardsService, equipmentService, attendanceService, salaryService } from '../services/storage';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const salaries = salaryService.getAll();

    setStats({
      onDuty: guards.filter(g => g.status === 'Active').length,
      present: attendance.filter(a => a.status === 'Present').length,
      absent: attendance.filter(a => a.status === 'Absent').length,
      billing: salaries.reduce((sum, s) => sum + (s.amount || 0), 0),
